...
func installPluginUnderTest(t *testing.T, plugName string) {
	pluginDir := path.Join(testPluginPath, plugName)
...
func TestPluginSetupHook(t *testing.T) {
	...
	err = plug.SetUpPod("podName", "podNamespace", "dockerid2345")
	if err != nil {
	...
	expectedOutput := "setup podName podNamespace dockerid2345"
...
func TestPluginTearDownHook(t *testing.T) {
	...
	err = plug.TearDownPod("podName", "podNamespace", "dockerid2345")
	if err != nil {
		t.Errorf("Expected nil")
	}
	// check output of setup hook
	...
	expectedOutput := "teardown podName podNamespace dockerid2345"
	if string(output) != expectedOutput {
